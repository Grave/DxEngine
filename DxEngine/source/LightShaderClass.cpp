...
bool LightShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	bool result = InitializeShader(device, hwnd, L"../DxEngine/data/light.vs", L"../DxEngine/data/light.ps");
...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	/*
	One of the major changes to the shader initialization is here in the polygonLayout. We add a third element for
	the normal vector that will be used for lighting. The semantic name is NORMAL and the format is the regular
	DXGI_FORMAT_R32G32B32_FLOAT which handles 3 floats for the x, y, and z of the normal vector. The layout will
	now match the expected input to the HLSL vertex shader.
	*/
	...
	constant buffers, if they are not multiples of 16 you need to pad extra space
	on to the end of them or the CreateBuffer function will fail. In this case the 
	constant buffer is 28 bytes with 4 bytes padding to make it 32.
	*/
...
bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix
